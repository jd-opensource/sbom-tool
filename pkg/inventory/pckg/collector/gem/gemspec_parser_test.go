...
func TestGemSpecParser_Parse(t *testing.T) {
	type args struct {
		path string
	}
	tests := []struct {
		...
		{
			"case-1",
			args{path: "test_material/test.gemspec"},
			[]model.Package{
...
func TestFindSubString(t *testing.T) {
	...
	specVar := findSub(specVarReg, " Gem::Specification.new  do  | spec1  | ")
	...
	dependency := findSubStringArray(dependencyReg, " spec.add_dependency \"example-gem\", \"~> 1.0\" ")
	...
	license := findSubStringArray(licensesReg, " spec.licenses = ['MIT', 'Apache-2'] ")
	assert.Equal(t, 2, len(license))
	if len(license) == 2 {
		assert.Equal(t, "MIT", license[0])
		assert.Equal(t, "Apache-2", license[1])
	}
